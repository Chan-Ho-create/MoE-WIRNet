...
parser.add_argument('--generated_images_path', default='weatherbench_output/RAIN/', help='Generated / Restored / Recovered images path')
parser.add_argument('--target_path', default='/root/autodl-tmp/Dataset/test/HAZE/target', help='Ground truth path')
parser.add_argument('--Score_save_path', default='./metrics/HAZE', help='save the psnr, ssim and lpips score by txt')
...
def main():
    ...
    if not os.path.exists(args.Score_save_path):
        print('NO_path to sava the Score,Making....')
        os.makedirs(args.Score_save_path)
    else:
        print("The Score_path has existed")
    PS_txt = open(os.path.join(args.Score_save_path, os.path.split(args.generated_images_path)[-1]) + '.txt', 'a')
    ...
    for i, img_path in enumerate(img_list):
        ...
        print('{:4} \tPSNR: {:.4f} dB, \tSSIM: {:.4f},\tLPIPS: {:.4f}'.format(
